...
`Spin whole text` rotates the complete placement as one rigid group around its centre.
Choose the local X, Y or Z axis; the animated character preview shows how the complete
text will turn in game. Character spacing and relative orientation remain unchanged.
`Frequency` controls the number of full rotations per second. Spin settings share the
existing appearance JSON, so they require no migration. Older per-character spin data
loads using its first enabled axis and applies that axis to the complete text.
